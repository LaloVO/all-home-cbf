...
const Desarrollos = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Desarrollos | {user?.nombre_usuario ?? 'All Home Bienes Raíces'}</title>
        <meta
          name="description"
          content="Conoce los desarrollos y preventas exclusivas curadas por All Home Bienes Raíces."
        />
      </Helmet>

      <Navbar />

      <main className="pt-28 min-h-screen bg-surface pb-24">
        <div className="px-6 md:px-12 max-w-[90rem] mx-auto">

          {/* Editorial header */}
          <div className="mb-16 max-w-2xl">
            <span className="text-accent text-[10px] uppercase tracking-[0.25em] font-sans font-bold block mb-4">
              Portafolio de Preventas
            </span>
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl text-primary font-light leading-[1.05]">
              Desarrollos
              <br />
              <span className="italic text-accent">en construcción</span>
            </h1>
            <p className="font-sans text-sm text-primary/70 font-light mt-6 leading-relaxed">
              ...
              {[...Array(4)].map((_, i) => (
                <div key={i} className="rounded-3xl overflow-hidden bg-white/40 border border-white/30 animate-pulse">
                  ...
                  <div className="p-7 space-y-3">
                    <div className="h-2.5 bg-primary/10 rounded-full w-1/3" />
                    <div className="h-5 bg-primary/10 rounded-full w-2/3" />
          ...
          {!isLoading && desarrollos.length === 0 && (
            <div className="flex flex-col items-center justify-center py-28 gap-6 text-center border border-dashed border-primary/20 bg-white/30 rounded-3xl">
              <div className="w-16 h-16 rounded-full bg-white border border-primary/20 flex items-center justify-center">
                <Building2 className="w-6 h-6 text-accent" />
              </div>
              <p className="font-serif text-2xl text-primary font-light max-w-sm">
                Nuevos desarrollos en curaduría
              </p>
              <p className="font-sans text-xs text-primary/60 max-w-xs">
                Muy pronto encontrarás aquí las preventas exclusivas seleccionadas por nuestro equipo.
              </p>
              <Link
                to="/mapa"
                className="mt-2 inline-flex items-center gap-2 px-6 py-3 border border-accent text-accent text-[10px] uppercase tracking-widest font-sans font-bold hover:bg-accent hover:text-white transition-colors duration-300 rounded-full"
              >
